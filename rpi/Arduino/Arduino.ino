...
short int NUM_OF_SENSOR_READINGS_TO_TAKE = 15;                  // getDistance()
short int NUM_OF_SENSOR_READINGS_TO_TAKE_CALIBRATION = 5;       // calAngle() and calDistance()
short int COMMAND_DELAY = 60;


const float WHEELCCF = 2 * PI * 3;
//define WHEELCCF = (2 * PI * 3)f;
...
float sensorCaliDistance(int sensor)
{
  ...
  float voltage, middle;
  
  for (int i = 0; i < NUM_SENSOR_READINGS_CALI; i++)
  {
    caliReading[i] = analogRead(sensor);
  }
  ...
  {
    case FL:
      return -6.3295 * pow(voltage, 3) + 45.689 * voltage * voltage - 113.84 * voltage + 101.02;
      
    case FR:
      return -1.7829 * pow(voltage, 3) + 14.583 * voltage * voltage - 43.971 * voltage + 49.982;

    case RF:
      return -1.7829 * pow(voltage, 3) + 14.583 * voltage * voltage + -43.971 * voltage + 49.982;
      
    case RB:
      return -3.3307 * pow(voltage, 3) + 24.122 * voltage * voltage - 62.573 * voltage + 63.155; //doenst works when distance less than 2.5cm
      
    case FC:
      return -1.6918 * pow(voltage, 3) + 13.651 * voltage * voltage - 40.899 * voltage + 47.82;
  }
